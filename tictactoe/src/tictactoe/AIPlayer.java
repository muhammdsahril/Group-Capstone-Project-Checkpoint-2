...
public class AIPlayer {
    ...
    public int getBestMove(Board board) {
        ...
        int bestCol = -1; // Default ke nilai tidak valid
    
        for (int col = 0; col < Board.COLS; col++) {
            if (board.getLowestEmptyRow(col) != -1) { // Periksa apakah kolom valid
                ...
                if (score > bestScore) {
                    bestScore = score;
                    bestCol = col; // Simpan kolom terbaik
                }
            }
        }
    
        // Jika tidak ada langkah valid, kembalikan nilai default (hindari error)
        return bestCol == -1 ? 0 : bestCol; // Pilih kolom pertama sebagai fallback
    }
    ...
    private int minimax(Board board, int depth, boolean isMaximizing, int alpha, int beta) {
        ...
        if (depth >= MAX_DEPTH) return evaluateBoard(board); // Batasi kedalaman
